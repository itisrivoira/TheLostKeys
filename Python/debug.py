...
class Debug():
    def log(self):
        ...
        if GLOB.Debug:
            ...
            KEY_TEXT = main.get_font(12*int(GLOB.MULT)).render(key, True, "#ffffff", "#00000048")
            ...
            POSP_TEXT = main.get_font(3*int(GLOB.MULT)).render("Player - x/y: "+str(int(main.player.getPositionX()/GLOB.MULT))+" | "+str(int(main.player.getPositionY()/GLOB.MULT)), True, "Red")
            POSP_RECT = POS_TEXT.get_rect(center=(70*GLOB.MULT, 40*GLOB.MULT))

            GLOB.screen.blit(POSP_TEXT, POSP_RECT)

            POSC_TEXT = main.get_font(3*int(GLOB.MULT)).render("Cam - x/y: "+str(int(main.cam.getPositionX()/GLOB.MULT))+" | "+str(int(main.cam.getPositionY()/GLOB.MULT)), True, "Blue")
            ...
            DIFF_TEXT = main.get_font(3*int(GLOB.MULT)).render("Secondi: "+str(GLOB.SecondDiffPos), True, "#fffb23")
            DIFF_RECT = DIFF_TEXT.get_rect(center=(50*GLOB.MULT, 50*GLOB.MULT))
            
            SEC_TEXT = main.get_font(3*int(GLOB.MULT)).render("Secondi Rit: "+str(GLOB.Val_sec), True, "#23fff4")
        ...
        if GLOB.ShowFps:
            FPS_TEXT = main.get_font(6*int(GLOB.MULT)).render("FPS: "+str(int(main.clock.get_fps())), True, "white")
            FPS_RECT = FPS_TEXT.get_rect(center=(GLOB.screen_width-40*GLOB.MULT, 20*GLOB.MULT))

            GLOB.screen.blit(FPS_TEXT, FPS_RECT)
            
        if GLOB.ShowCodice:
            COD_TEXT = main.get_font(6*int(GLOB.MULT)).render(str(GLOB.codice), True, "White")
            COD_RECT = COD_TEXT.get_rect(center=(GLOB.screen_width/2, 13*GLOB.MULT))
            
            COV_TEXT = main.get_font(6*int(GLOB.MULT)).render(str(GLOB.codice), True, "Black")
        ...
        if GLOB.ShowDropFrames and GLOB.Debug:
            ...
            DROP_TEXT = main.get_font(3*int(GLOB.MULT)).render("DROP "+str(100-int(main.clock.get_fps()*100/GLOB.FPS))+"%", True, "red")
